...
// need to make sure we are on a single node for this to work
    ...
    cout << "Probing device with ID=" << 0 << " on host " << HostName << ": " << DeviceProperties.name << endl;
    ...
    if (Cores == 0)  {
        Cores = 3072;
        cout << "Could not count cores! Your GPU is possibly too new. We'll take " << Cores << " but it doesn't really matter." << endl;
    }
    const int RealMaxGS = Cores/2;
    cout << "We are only going to examine grids smaller than " << RealMaxGS << " blocks, for absolutely no good reason." << endl;
